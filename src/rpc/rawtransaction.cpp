...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
	...
	{
		throw std::runtime_error(
            "getaddresstxids need address param or address param is not array\n"
        );
	}
	
    int startPos = 0;
    int endPos = 0;
	UniValue uniStart = NullUniValue;
	UniValue uniEnd = NullUniValue;
	uniStart = find_value(oparam, "start");
	uniEnd = find_value(oparam, "end");
	if (!uniStart.isNull() && uniStart.isNum() &&
	   !uniEnd.isNull() && uniEnd.isNum() )
	{
		startPos = uniStart.get_int();
		endPos = uniEnd.get_int();
	}
	...
	for (unsigned int i = 0; i < addres.size(); ++i) {
		...
		while (it != setpos.end()) {
			...
			for(const CTxIn& itIn:ptx->vin)
			{
				...
				CTxDestination addressIn;
				const Coin& coin = view.AccessCoin(itIn.prevout);
				if (!ExtractDestination(coin.out.scriptPubKey, addressIn))
				{
					LogPrintf("rawtransation 231:get destination from script pub key failed.\n");
					continue;
				}
