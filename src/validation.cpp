...
{
    return prime.GetPrimeBlockValue(nBits, consensusParams);
}
...
static void CheckForkWarningConditions()
{
    ...
    const CChainParams& chainparams = Params();
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip(), chainparams.GetConsensus()) * 6)))
    {
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    if (pfork && (!pindexBestForkTip || pindexNewForkTip->nHeight > pindexBestForkTip->nHeight) &&
            pindexNewForkTip->nChainWork - pfork->nChainWork > (GetBlockProof(*pfork, chainparams.GetConsensus()) * 7) &&
...
static bool WriteTxIndexDataForBlock(CValidationState& state,std::vector<std::pair<uint256, CDiskTxPos> > &vPosTxid,std::vector<std::pair<uint160, CExtDiskTxPos> > &vPosAddrid)
{
    if (!fTxIndex || !fAddrIndex) return true;
	
	// here sometimes can't use view.AccessCoin(txin.prevout) visit preout tx.
	// remove to ConnectBlock function 
	...
	//			{
	//				LogPrintf("coin is empty =============\n");
	//				LogPrintf("coin is empty =============\n");
	//				LogPrintf("coin is empty =============\n");
	//			}
	//			if (coin.IsSpent()) {
    //                std::string err("Previous output has been spent:\n");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
	...
	// store address in tx.vin here, some tx may can't access at the tail part
	CExtDiskTxPos pos(CDiskTxPos(pindex->GetBlockPos(), GetSizeOfCompactSize(block.vtx.size())), pindex->nHeight);
    std::vector<std::pair<uint256, CDiskTxPos> > vPosTxid;
    std::vector<std::pair<uint160, CExtDiskTxPos> > vPosAddrid;
    vPosTxid.reserve(block.vtx.size());
    ...
    if (!WriteTxIndexDataForBlock(state,vPosTxid,vPosAddrid))
        return false;
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    bool fCheck = false;
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast() && fCheck)
